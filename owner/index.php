<?php
include("../assets/template/owner/topNav.php");
?>
<br>
<div class="container large">
	<h5 class="black-text">Dashboard <span class="right">6/30/2016 &bull; 23:38</span></h5>
	<br>
	<br>
	<div class="row">

		<div class="col s12 m4 l2 hide-on-med-and-down">
			<p class="black-text">Contacts</p>
			<br>
			<p style="font-size:100%;"><span class="primary">Fertilizer Specilist</span> - <span class="primary">510-656-9196</span> Armando Hussein
			<br><span class="error">M-F 9am - 5pm</span></p>
		</div>


		<div class="col s12 m12 l8">
			<p>Displaying <span class="black-text">all activity <i class="fa fa-angle-down" aria-hidden="true"></i></span> by <span class="black-text">Date <i class="fa fa-angle-down" aria-hidden="true"></i></span></p>
			<br>
			<div class="row">
				<div class="col s10">
					<p>Carpet Cleaner Guy will come to <span class="primary">clean Unit 4</span> on 7/16/2017</p>
				</div>
				<div class="col s2">
					<p class="right">6/30/2017</p>
				</div>
			</div>
			<div class="row">
				<div class="col s10">
					<p>Esteban Perez at Unit 12 has Perez at Unit 12 hasPerez at Unit 12 hasPerez at Unit 12 has <span class="primary">leaky faucet </span> on 6/30/2017</p>
				</div>
				<div class="col s2">
					<p class="right">6/30/2017</p>
				</div>
			</div>
			<div class="row">
				<div class="col s10">
					<p>Esteban Perez at Unit 12 has <span class="primary">leaky faucet</span> on 6/30/2017</p>
				</div>
				<div class="col s2">
					<p class="right">6/30/2017</p>
				</div>
			</div>
			<div class="row">
				<div class="col s10">
					<p>Andres Reyes at Unit 4 needs to <span class="primary">renew his renters insurance</span> on 9/11/2017</p>
				</div>
				<div class="col s2">
					<p class="right">6/30/2017</p>
				</div>
			</div>
			<div class="row">
				<div class="col s10">
					<p>Nickolas Vaishampayan at Unit 21 has <span class="primary">my sink broke</span> on 6/30/2017</p>
				</div>
				<div class="col s2">
					<p class="right">6/30/2017</p>
				</div>
			</div>
			<div class="row">
				<div class="col s10">
					<p>Esteban Perez at Unit 12 has <span class="primary">leaky faucet</span> on 6/30/2017</p>
				</div>
				<div class="col s2">
					<p class="right">6/30/2017</p>
				</div>
			</div>
			<div class="row">
				<div class="col s10">
					<p>Esteban Perez at Unit 12 has <span class="primary">leaky faucet</span> on 6/30/2017</p>
				</div>
				<div class="col s2">
					<p class="right">6/30/2017</p>
				</div>
			</div>
			<div class="row">
				<div class="col s10">
					<p>Esteban Perez at Unit 12 has <span class="primary">leaky faucet</span> on 6/30/2017</p>
				</div>
				<div class="col s2">
					<p class="right">6/30/2017</p>
				</div>
			</div>
		</div>
		<div class="col s12 m6 hide-on-large-only">
			<p class="black-text">My Goals</p>
			<br>
			<p>There is stuff that goes here</p>
		</div>
		<style>
			.row2 {
				margin-bottom:0px;
			}
		</style>
		<div class="col s12 m6 l2">
			<p class="black-text">My Tasks</p>
			<br>
			<!--this thing-->

			<div class="row" class="margin-bottom:0px;">
				<div class="col s1" style="padding:0;">
					<input type="checkbox" name="id1" value="1">
				</div>
				<div class="col s11" style="padding:0;">
					<p class="primary" style="font-size:100%;">Call the Plumber to unclog Esteban's Toilet at Unit 12</p>
					<p style="font-size:100%;">9/19/2017 &bull; <wbr><span class="primary">510-570-5088</span></p>
				</div>
			</div>
			<div class="row" class="margin-bottom:0px;">
				<div class="col s1" style="padding:0;">
					<input type="checkbox" name="id1" value="1">
				</div>
				<div class="col s11" style="padding:0;">
					<p class="primary" style="font-size:100%;">Call the Plumber to unclog Esteban's Toilet at Unit 12</p>
					<p style="font-size:100%;">9/19/2017 &bull; <wbr><span class="primary">510-570-5088</span></p>
				</div>
			</div>
			<div class="row" class="margin-bottom:0px;">
				<div class="col s1" style="padding:0;">
					<input type="checkbox" name="id1" value="1">
				</div>
				<div class="col s11" style="padding:0;">
					<p class="primary" style="font-size:100%;">Call the Plumber to unclog Esteban's Toilet at Unit 12</p>
					<p style="font-size:100%;">9/19/2017 &bull; <wbr><span class="primary">510-570-5088</span></p>
				</div>
			</div>
			<div class="row" class="margin-bottom:0px;">
				<div class="col s1" style="padding:0;">
					<input type="checkbox" name="id1" value="1">
				</div>
				<div class="col s11" style="padding:0;">
					<p class="primary" style="font-size:100%;">Call the Plumber to unclog Esteban's Toilet at Unit 12</p>
					<p style="font-size:100%;">9/19/2017 &bull; <wbr><span class="primary">510-570-5088</span></p>
				</div>
			</div>
			<div class="row" class="margin-bottom:0px;">
				<div class="col s1" style="padding:0;">
					<input type="checkbox" name="id1" value="1">
				</div>
				<div class="col s11" style="padding:0;">
					<p class="primary" style="font-size:100%;">Call the Plumber to unclog Esteban's Toilet at Unit 12</p>
					<p style="font-size:100%;">9/19/2017 &bull; <wbr><span class="primary">510-570-5088</span></p>
				</div>
			</div>

			<p class="black-text">My Upcomming Appointments</p>
			<br>
			<p>There is stuff that goes here</p><br>
			<p>There is stuff that goes here</p><br>
			<p>There is stuff that goes here</p><br>
			<p>There is stuff that goes here</p><br>
			<p>There is stuff that goes here</p><br>
			<p>There is stuff that goes here</p><br>
		</div>
	</div>
</div>




<?php
include("../assets/template/owner/botNav.php");
?>
